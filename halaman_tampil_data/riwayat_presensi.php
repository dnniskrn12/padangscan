            ...
            <table id="example2" class="table table-bordered table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Nomor Karyawan</th>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td class="text-center td-fit" style="vertical-align: middle;">
                                <?= indonesiaDate($row['tanggal']) ?></td>
                            <td class="text-center td-fit" style="vertical-align: middle;">
                                <?= $row['status'] == 'Hadir' ? $row['waktu'] : '-'; ?></td>
                            <td class="text-center td-fit" style="vertical-align: middle;"><?= $row['no_karyawan'] ?></td>
                            <td style="vertical-align: middle;"><?= $row['nama'] ?></td>
                            <td class="text-center" style="vertical-align: middle;">
                                <?= $row['status'] == 'Hadir' ? $row['jenis'] : '-'; ?></td>
                            <td class="text-center" style="vertical-align: middle;"><?= $row['status']; ?></td>
                            <td class="text-center td-fit">
                                <a href="?page=riwayat_presensi&method=edit&id=<?= $row['id'] ?>"
                                    class="btn btn-sm btn-warning text-white">Edit</a>
                                <form action="?page=riwayat_presensi&method=hapus&id=<?= $row['id'] ?>" method="POST"
                                    class="d-inline">
